...
#define INF INT_MAX
    ...
    static int Depth = 0, QLength = 0;
    static char LastNode = 'A';
    struct ClsDkl {
        int D; int K; int L;
        ClsDkl() : D(-1), K(0), L(0) {}
    ...
    struct ClsNode
    {
        ...
        vector<vector<int>> W;
        int n, Bound, Level;
        int Index; //Index in the complete binary tree
        char Label;
        ClsNode() : n(0), Bound(0), Level(0), Index(0), Label('A') {}
    ...
    {
        printf("Node: %c\n", Label);
        printf("n: %d, Level: %d, Index: %d, Bound: %d\n", n, Level, Index, Bound);
        printf("Path: ");
...
void TestBBTSPt()
{
    vector<vector<vector<int>>> w =
    {
        ...
        //Foundations of Algorithms, Sec. 6.2
        //Optimal dist: 30
    ...
    for (int i = 0; i < n; i++) {
        ...
        printf("\n");
        printf("Depth: %d, LastNode: %c, QLength: %d\n", Depth, LastNode, QLength);
    }
}
#include "..\..\..\..\include\headers.h"
void TestBBTSPRandomt()
{
#define N 5
